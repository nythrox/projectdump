...
import 'achievements_items.dart';
import 'achievements_items_2.dart';
            ...
            FutureBuilder<List<Medal>>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.hasError)
                    return Center(child: Text("Ocorreu um erro. Por favor, tente novamente."));
                  if (snapshot.hasData && snapshot.data.isEmpty)
                    return Center(child: Text("Não há nenhuma medalha."));
                  if (snapshot.hasData)
...
class ProgressCircleAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: (){
        Navigator.push(context, MaterialPageRoute(builder: (context)=> AchievementsItems(userId: 1,medalId: medal.id,)));
      },
        ...
        child:
          ...
          Container(
            ...
            child: CircleAvatar(
              ...
              backgroundColor: Colors.white,
              backgroundImage:
                  NetworkImage('${NetworkUtils.urlImage}${medal.image}'),
            ),
          ),
          Container(
            margin: EdgeInsets.only(top: 10.0, bottom: 30.0),
