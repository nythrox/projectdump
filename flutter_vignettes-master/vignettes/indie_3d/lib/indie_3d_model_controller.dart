...
abstract class VertexController extends ChangeNotifier {
  ...
  abstract class VertexSceneController implements VertexController {
    Future<void> loadDependencies(BuildContext context);
  ...
  class VertexDefaultController extends ChangeNotifier
    ...
    VertexDefaultController(this._meshPaths, this._instancesInfo) : _meshes = [];

    VertexDefaultController.fromVertexMeshes(this._meshes, this._instancesInfo)
        : _meshPaths = null;
    ...
    void _setTransform() {
      for (int i = 0; i < _instances.length; i++) {
          ...
          final modelMatrix = vec32.Matrix4.compose(_instances[i].position,
              _instances[i].rotation, _instances[i].scale);
  ...
  class RandomVertexController extends VertexDefaultController {
    ...
    RandomVertexController(
        List<ObjPath> _meshPaths, List<InstanceInfo> _instancesInfo)
        : super(_meshPaths, _instancesInfo);

    RandomVertexController.fromVertexMeshes(
        List<VertexMesh> meshes, List<InstanceInfo> _instancesInfo)
        : super.fromVertexMeshes(meshes, _instancesInfo);
  ...
  class CameraVertexController extends VertexDefaultController {
    CameraVertexController(List<ObjPath> paths, List<InstanceInfo> instancesInfo)
        : super(paths, instancesInfo);
    CameraVertexController.fromVertexMeshes(
        List<VertexMesh> vertexMeshes, List<InstanceInfo> instancesInfo)
        : super.fromVertexMeshes(vertexMeshes, instancesInfo);
